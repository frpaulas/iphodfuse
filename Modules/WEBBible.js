var BookName = require('./book_names')
  ;
function getReading(ref) {
  var [book, chap, vsFrom, vsTo] = parseRef(ref)
    , title = BookName.book_title(book)
    ...
    ;
  return {title: title, chap: chap, vsFrom: vsFrom, vsTo: vsTo, vss: vss};
};

function parseRef(ref) {
  var [book, temp] = ref.split(" ")
    , [chap, temp] = temp.split(":")
    , [vsFrom, vsTo] = temp.split("-")
    ;
  return [book, chap, vsFrom, vsTo];
}
