...
// /Users/paulsutcliffe/Library/Application Support/Fusetools/Packages/UnoCore/0.47.13/source/uno/graphics/$.uno
// -------------------------------------------------------------------------------------------------------------
...
static void Texture2D_build(uType* type)
{
    ...
    ::STRINGS[3] = uString::Const("Texture2D");
    ::STRINGS[4] = uString::Const("Texture is immutable and cannot be updated");
    ::STRINGS[5] = uString::Const("GL_OES_texture_npot");
...
{
    if (IsDisposed())
        U_THROW(::g::Uno::ObjectDisposedException::New4(::STRINGS[3/*"Texture2D"*/]));
    else
...
{
    if (Format() == 0)
        U_THROW(::g::Uno::InvalidOperationException::New5(::STRINGS[4/*"Texture is ...*/]));
    else
    {
...
bool Texture2D::HaveNonPow2Support()
{
    ...
    {
        Texture2D::_haveNonPow2Support_ = (Texture2D::CheckGLES3Support() || Texture2D::CheckExtensionSupport(::STRINGS[5/*"GL_OES_text...*/]));
...
{
    ::STRINGS[6] = uString::Const("VideoTexture");
    type->SetInterfaces(
...
{
    if (IsDisposed())
        U_THROW(::g::Uno::ObjectDisposedException::New4(::STRINGS[6/*"VideoTexture"*/]));
    else
        ::g::OpenGL::GL::DeleteTexture(GLTextureHandle());
